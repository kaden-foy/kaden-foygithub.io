...
width_num = 100 # display width (pixels) [#]
width_dim = np.array([-32,32]) # display width (size) [mm]

height_num = 75 # display height (pixels) [#]
...
frame_num = 6 # number of frames from 0-180 deg

obj_func = np.zeros([width_num,height_num,frame_num]) # initize array
...
for frame in range(frame_num):
    ...
    theta = np.deg2rad(180/(frame_num+1))
    ...
    for w in range(width_num):
        for h in range(height_num):
            ...
            #if (len(dispacement[:,0])%2==1):
                #print("something wrong with ray ", ray)
            obj_func[w,h,frame] = np.sqrt(np.sum(dispacement[::2,0]**2 + dispacement[::2,1]**2))
            ray += 1

    stop = timeit.default_timer() #stop timer
    print("Generate object functions in ",stop-start," [s]")

plt.imshow(obj_func[:,:,0],cmap='gray')
plt.show()